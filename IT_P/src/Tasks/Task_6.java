...
public class Task_6 {
    ...
    public static String translateWord(String word) {
        ...
        StringBuilder pieceOfPig = new StringBuilder();
        pig.reverse();
        boolean flag = true;
        for (char c : word.toCharArray()) {
            if (!ISAlphabetic.contains(String.valueOf(c))) {
                pieceOfPig.append(c);
                int lastIndex = pig.length() - 1;
                pig.deleteCharAt(lastIndex);
                flag = false;
            } else {
                pig.reverse();
                break;
        ...
        if (flag) return pig.append(pieceOfPig).append("yay").toString();
        else return pig.append(pieceOfPig).append("ay").toString();
    }
