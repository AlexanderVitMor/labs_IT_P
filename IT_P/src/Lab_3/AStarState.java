...
public class AStarState
{
    ...
    public Waypoint getMinOpenWaypoint()
    {
        ...
        for (Location key : openWaypoints.keySet()){
            Waypoint waypoint = openWaypoints.get(key);
            float costNow = waypoint.getTotalCost();
            if (costNow <= bestCost){
                bestCost = costNow;
    ...
    {
        Location locationNewWP = newWP.getLocation();

        if (openWaypoints.containsKey(locationNewWP)){
            float newCostPrevious = newWP.getPreviousCost();
            float lastCostPrevious = openWaypoints.get(locationNewWP).getPreviousCost();
            if(newCostPrevious <= lastCostPrevious) {
                openWaypoints.put(locationNewWP, newWP);
                return true;
            }
            else return false;
        }
        else {
            openWaypoints.putIfAbsent(locationNewWP, newWP);
            return true;
        }
    }
